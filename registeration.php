...
<?php
  ...
  $query = "SELECT max(idAnggota) as maxAnggota FROM t_anggota";
  $hasil = mysqli_query($koneksi,$query);
  $data = mysqli_fetch_array($hasil);
  $kodeAnggota = $data['maxAnggota'];

  $noUrut = (int) substr($kodeAnggota, 3, 3);
   
  $noUrut++;
   
  $char = "ANG";
  $kodeAnggota = $char . sprintf("%03s", $noUrut);
...
<?php  
    ...
    $tambah = mysqli_query($koneksi,"insert into t_anggota (idAnggota, Nama, Alamat, Email, Username, Password) values ('$id','$nama','$alamat','$email','$username','$password')")or die(mysqli_error($koneksi));

    $tambah_login = mysqli_query($koneksi,"insert into t_login (idAdmin, username, password, status) values ('$id','$username','$password', 'user')"); 
        session_start();
    if($tambah){    
        echo "
...
<body style="background-image: url('Gambar/register.jpg');">
    <div class="container">
        <div class="row text-center  ">
            <div class="col-md-12">
                <br /><br />
                <h2 style="color: white">Member : Register Form</h2>
         ...
         <div class="row">
                ...
                <div class="col-md-4 col-md-offset-4 col-sm-6 col-sm-offset-3 col-xs-10 col-xs-offset-1">
                        ...
                        <strong>  New Member ? Register Yourself </strong>  
                            </div>
...
<br/>
                                        <div class="form-group input-group">
                                            ...
                                            <input name="idanggota" type="text" class="form-control" placeholder="Your ID" required="" oninvalid="this.setCustomValidity('Please Enter Your ID')"
                                                oninput="setCustomValidity('')" value="<?= $kodeAnggota ?>" readonly/>
                                        </div>
                                        <div class="form-group input-group">
